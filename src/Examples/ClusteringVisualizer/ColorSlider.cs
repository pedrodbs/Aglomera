...
//    Source: https://www.codeproject.com/Articles/17395/Owner-drawn-trackbar-slider
// </summary>
// ------------------------------------------
...
{
    /// <summary>
    ///     Encapsulates control that visually displays certain integer value and allows user to change it within desired range.
    ...
    [Designer(typeof(ClusteringVisualizer.ColorSlider), typeof(IRootDesigner))]
    public partial class ColorSlider : UserControl
    {
        ...
        public int ThumbSize
        {
            ...
            {
                if ((value > 0) &
                    (value < (this.barOrientation == Orientation.Horizontal
        ...
        public GraphicsPath ThumbCustomShape
        {
            ...
            set
            {
                ...
                thumbSize = (int) (barOrientation == Orientation.Horizontal
                                ? value.GetBounds().Width
                                : value.GetBounds().Height) + 1;
                Invalidate();
            }
        }
        ...
        private readonly Color[,] aColorSchema =
        {
            ...
            {
                Color.White, Color.Gainsboro, Color.Silver, Color.Red,
                Color.DarkRed, Color.Gainsboro, Color.Coral,
                Color.LightCoral
            },
            {
                Color.White, Color.Gainsboro, Color.Silver, Color.GreenYellow,
                Color.Yellow, Color.Gold, Color.Orange,
                Color.OrangeRed
            },
            {
                Color.White, Color.Gainsboro, Color.Silver, Color.Red,
                Color.Crimson, Color.Gainsboro, Color.DarkViolet, Color.Violet
            }
        };
            ...
            Color barPenColorPaint, Color elapsedOuterColorPaint, Color elapsedInnerColorPaint)
        {
            try
            {
                ...
                {
                    int TrackX = (this.trackerValue - this.barMinimum) * (this.ClientRectangle.Width - this.thumbSize) /
                                 (this.barMaximum - this.barMinimum);
                    this.ThumbRect = new Rectangle(TrackX, 1, thumbSize - 1, ClientRectangle.Height - 3);
                }
                else
                {
                    int TrackY = (this.trackerValue - this.barMinimum) *
                                 (this.ClientRectangle.Height - this.thumbSize) / (this.barMaximum - this.barMinimum);
                    this.ThumbRect = new Rectangle(1, TrackY, ClientRectangle.Width - 3, thumbSize - 1);
                }

                //adjust drawing rects
                barRect = ClientRectangle;
                thumbHalfRect = this.ThumbRect;
                ...
                if (barOrientation == Orientation.Horizontal)
                {
                    ...
                    elapsedRect = barRect;
                    elapsedRect.Width = this.ThumbRect.Left + thumbSize / 2;
                }
                else
                {
                    ...
                    elapsedRect = barRect;
                    elapsedRect.Height = this.ThumbRect.Top + thumbSize / 2;
                }

                //get thumb shape path 
                GraphicsPath thumbPath;
                if (thumbCustomShape == null)
                    thumbPath = CreateRoundRectPath(this.ThumbRect, thumbRoundRectSize);
                else
                {
                    thumbPath = thumbCustomShape;
                    Matrix m = new Matrix();
                    m.Translate(this.ThumbRect.Left - thumbPath.GetBounds().Left,
                        this.ThumbRect.Top - thumbPath.GetBounds().Top);
                ...
                using (
                    LinearGradientBrush lgbBar =
                        new LinearGradientBrush(barHalfRect, barOuterColorPaint, barInnerColorPaint,
                            gradientOrientation)
                )
                {
        ...
        {
            base.OnMouseMove(e);
            mouseInThumbRegion = IsPointInRect(e.Location, this.ThumbRect);
            if (Capture & (e.Button == MouseButtons.Left))
            {
                ...
                p -= margin;
                float coef = (this.barMaximum - this.barMinimum) /
                             (float)
        ...
        protected override void OnKeyUp(KeyEventArgs e)
        {
            ...
            switch (e.KeyCode)
            {
                case Keys.Down:
                case Keys.Left:
                    SetProperValue(Value - (int) this.SmallChange);
                ...
                case Keys.End:
                    ...
                    break;
                case Keys.PageDown:
                    SetProperValue(Value - (int) this.LargeChange);
        ...
        public static Color[] DesaturateColors(params Color[] colorsToDesaturate)
        {
            ...
            for (int i = 0; i < colorsToDesaturate.Length; i++)
            {
                ...
                int gray =
                    (int) (colorsToDesaturate[i].R * 0.3 + colorsToDesaturate[i].G * 0.6 +
