...
namespace Agnes.Evaluation
{
    ...
    /// </remarks>
    public class FMeasure<TInstance, TClass> : IExternalCriterion<TInstance, TClass>
            ...
            for (var i = 0; i < clusterSet.Count; i++)
            {
                ...
                foreach (var instance in clusterGroup)
                {
                    ...
                    for (var j = i + 1; j < clusterSet.Count; j++)
                        falseNegatives += clusterSet[j].Count(instance2 => instanceClass.Equals(instanceClasses[instance2]));
                }
